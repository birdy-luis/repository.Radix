...
ADDON_PATH            = xbmc.translatePath('special://home/addons/plugin.video.ask/')
source_file2          = ADDON_PATH + 'opendir.xml'
PATH                  = 'Index Scraper'
VERSION               = '0.0.1'
addon_id              = 'plugin.video.ask'
...
baseurl               = base64.b64decode(b'aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL192NC9tYWlubWVudS54bWw=')
searchicon            = xbmc.translatePath(os.path.join('special://home/addons/' + addon_id, 'search.png'))
images                = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.ask/resources'))

########################################################################

SETTINGS_FILE        =  xbmc.translatePath(os.path.join('special://home/addons/plugin.video.ask/resources/settings.xml'))
DATA_SETTINGS_FILE   =  xbmc.translatePath(os.path.join('special://home/userdata/addon_data/plugin.video.ask/settings.xml'))

net = Net(user_agent='ask')
headers = {
...
####################### INDEX SOURCES FAVOURITES START
...
def Read_Favourite(name,url,choice,mode):
    ...
    for url,name,mode in Fav_Regex:
        ...
        Menu('[COLOR gold][B]If this list is empty you need to add your Favourites![/B][/COLOR]','','','','','','','')
        Menu('[COLOR crimson][B]By bringing up context menu then adding to your favourites![/B][/COLOR]','','','','','','','')
        Menu('[COLOR gold][B]Press "C" or "Menu" or "Right click" to bring up the context menu[/B][/COLOR]','','','','','','','')
        Menu('[COLOR crimson][B]Them simply add you favourite to the revelant section movies,TV,music[/B][/COLOR]','','','','','','','')

    
def Favourites_menu():
    Menu('[COLOR gold][B]Favourite Movies[/B][/COLOR]','',61,ICON,FANART,'','',1)
    Menu('[COLOR crimson][B]Favourite Tv Shows[/B][/COLOR]','',61,ICON,FANART,'','',2)
    Menu('[COLOR gold][B]Favourite Music[/B][/COLOR]','',61,ICON,FANART,'','',3)

####################### INDEX SOURCES FAVOURITES FINISH 
           

####################### MAIN REGEX LOOP START
...
def Main_Loop(url):
    ...
    for url2,name in match:
        ...
        if '..' in url3:
            ...
            Menu((name).replace('/',''),url3,59,ICON,FANART,'','','')
        else:
            Clean_name(name,url3)

####################### MAIN REGEX LOOP FINISH
################################### TIDY UP NAME #############################
...
def Clean_name(name,url3):
    ...
    if ADDON.getSetting('Data')=='false':
        Play(clean_name,url3,58,ICON,FANART,'','','')
    ...
    for url in match:
        if 'title' in url:
            ...
            if not Pass_It in List:
                ...
                Play(clean_name,url3,58,IMAGE,BACKGROUND,DESCRIPTION,'','')
                List.append(Pass_It)
                setView('movies', 'INFO')
            else:
                pass
...
def Play(name,url,mode,iconimage,fanart,description,trailer,choice,showcontext=True,allinfo={}):
        ...
        if showcontext:
            ...
            if not name in movie_favourites_read:
                contextMenu.append(('Add to [COLOR gold][B]•[/B][/COLOR][COLOR gold][B]THE[/B][/COLOR] [COLOR crimson][B]BORROWERS[/B][/COLOR][COLOR gold][B]•[/B][/COLOR] Movie_Favourites','XBMC.RunPlugin(%s?choice=1&mode=3&name=%s&url=%s&iconimage=%s&fav_mode=%s)'
                         %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage), mode)))
            if not name in tv_favourites_read:
                contextMenu.append(('Add to [COLOR gold][B]•[/B][/COLOR][COLOR gold][B]THE[/B][/COLOR] [COLOR crimson][B]BORROWERS[/B][/COLOR][COLOR gold][B]•[/B][/COLOR] TV_Favorites','XBMC.RunPlugin(%s?choice=2&mode=3&name=%s&url=%s&iconimage=%s&fav_mode=%s)'
                         %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage), mode)))
            if not name in music_favourites_read:
                contextMenu.append(('Add to [COLOR gold][B]•[/B][/COLOR][COLOR gold][B]THE[/B][/COLOR] [COLOR crimson][B]BORROWERS[/B][/COLOR][COLOR gold][B]•[/B][/COLOR] Music_Favorites','XBMC.RunPlugin(%s?choice=3&mode=3&name=%s&url=%s&iconimage=%s&fav_mode=%s)'
                         %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage), mode)))              
...
HOME       =  base64.b64decode('aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL192NC9BZHVsdEFyZWEvQWR1bHRBcmVhLnhtbA==')
NOTICE     =  base64.b64decode('aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL25vdGljZS50eHQ=')
TABLE      =  base64.b64decode('aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL192NC90YWJsZS50eHQ=')
CONTACT    =  base64.b64decode('aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL0NvbnRhY3RVUy50eHQ=')
REPO       =  xbmc.translatePath(os.path.join('special://home/addons','ask.repository'))
AddonTitle = "[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR]"

if not os.path.exists(REPO):
	choice = xbmcgui.Dialog().yesno(AddonTitle,'[COLOR yellow]You MUST Have The [B][COLOR blue][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR yellow][B]  Repo[/B][/COLOR] Installed For This Addon To Work. Would You Like To Install The Repository Now?[/COLOR]','',yeslabel='[B][COLOR white]YES[/COLOR][/B]',nolabel='[B][COLOR grey]NO[/COLOR][/B]')
	if choice == 1:
		path = xbmc.translatePath(os.path.join('special://home/addons','packages'))
		if not os.path.exists(path):
			os.makedirs(path)
		url = base64.b64decode(b'aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL0EuUy5LLUtvZGktQWRkb24vYXNrLnJlcG9zaXRvcnkuemlw')
		dp = xbmcgui.DialogProgress()
		dp.create(AddonTitle,"","","Downloading [COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] [COLOR white] Repo[/COLOR]")
		...
		time.sleep(2)
		dp.update(0,"","Installing [COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] [COLOR white] Repo[/COLOR] Please Wait","")
		extract.all(lib,addonfolder,dp)
		xbmc.executebuiltin("UpdateAddonRepos")
		xbmc.executebuiltin("UpdateLocalAddons")	

	
addon         = xbmcaddon.Addon('plugin.video.ask')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.ask]: %s" %(addon_version, string))

ADULTAREA = base64.b64decode('aHR0cDovL2FzazRhY2Nlc3MuY29tL3VybHMvc2VhcmNo')
def makeRequest(url, headers=None):

        if "AdultArea" in url:
            keyb = xbmc.Keyboard('', "[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR white] [/COLOR] [COLOR red][B]ADULT PASSWORD[/B][/COLOR]")
            keyb.doModal()
            if (keyb.isConfirmed()):
            ...
            if not password==passkey:
                dialog = xbmcgui.Dialog()
                dialog.ok("[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR]","Sorry, the password you entered was incorrect.","You can get the password at [COLOR lime]www.ask4access.com/members[/COLOR] Press OK and try again.")
                sys.exit(1)

        if "_TEST-FILES" in url:
            keyb = xbmc.Keyboard('', "[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR white] [/COLOR] [COLOR red][B]Test Area : Please Enter The Test Area Password[/B][/COLOR]")
            keyb.doModal()
            if (keyb.isConfirmed()):
                password2= keyb.getText()
                password = base64.b64encode(password2)
            ...
            if not password==passkey:
                dialog = xbmcgui.Dialog()
                dialog.ok("[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR]","[COLOR yellow][B]Sorry, the password you entered was incorrect.[/B][/COLOR]",".....")
                sys.exit(1)
	...
	if "Acestream" in url:
        ...
        if "table" in url:
            table = requests.get(TABLE)
            TextBoxesPlain("%s" % table.text)
            sys.exit(1)
			
        if "ContactUS" in url:
            ContactUS = requests.get(CONTACT)
            TextBoxesPlain("%s" % ContactUS.text)
            sys.exit(1)

        try:
            if headers is None:
                headers = {'User-agent' : 'ask'}
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification([COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR],[COLOR white][B]SERVER:[/COLOR] [COLOR red]OFFLINE[/COLOR][/B] - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification([COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR],[COLOR white][B]SERVER:[/COLOR] [COLOR red]OFFLINE[/COLOR][/B] - "+str(e.reason)+",10000,"+icon+")")
...
# open directory menu item below
...
def CPIndex():
    ...
    try:
        f = requests.get(NOTICE)
        TextBoxesPlain("%s" % f.text)
    except: pass
	
    addon_log("CPIndex")
    getData(ADULTAREA,'')
    addDir("[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR yellow][B] Movie and TV Libraries[/B][/COLOR][COLOR yellow][B]  •[/B][/COLOR]",base64.b64decode(b'aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL192NC9vcGVuZGlyLnhtbA=='),666,"","","","movies","",None,'source')
    addDir("[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR yellow][B] Login + Settings •[/B][/COLOR]",'',668,"","","","settings","",None,'source')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))


# Thanks to Midrral - Bob for fixing Search function. 	
def Search_loop():
	Dialog = xbmcgui.Dialog()
	Search_name = Dialog.input('[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR white] Search [/COLOR] ', type=xbmcgui.INPUT_ALPHANUM)
	if Search_name == '':
		return
	else:
		Search_title = Search_name.lower().replace(' ','')
        for url_to_open in [b'aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL192NC9MaXZlLVRWL0xpdmUtdHYxLnhtbA==', b'aHR0cDovL2FzazRhY2Nlc3MuY29tL2Fzay1rb2RpL192NC9zcG9ydHMvVG9kYXlzU3BvcnQueG1s']:
            url_to_open = str(base64.b64decode(url_to_open))
            HTML = getUrl(url_to_open)
            items = re.compile("<item>(.+?)</item>",re.DOTALL).findall(HTML)
            for item in items:
                name = get_tag_content(item, "title", "")
                url = get_tag_content(item, "link", "")
                external_link = get_tag_content(item, "externallink", "")
                img = get_tag_content(item, "thumbnail", "")
                if '<title>' in name:
                    continue
                if external_link and not external_link in ["http://no/", "http://nolink", "http://nolink/"]:
                    url = external_link
                ...
                if 'sublink' in url:
                    ...
                    if Search_title in name.lower().replace(' ',''):
                        if url.endswith("xml"):
                            addDir(name,url,1,img,FANART,'','','','')
                        else:
                            addLink(url, name,img,FANART,'','','','',None,'',1)
            HTML2 = getUrl('http://ask4access.com/ask-kodi/_v4/search.xml')
            items = re.compile("<item>(.+?)</item>",re.DOTALL).findall(HTML2)
            for item in items:
                name2 = get_tag_content(item, "title", "")
                url2 = get_tag_content(item, "link", "")
                external_link = get_tag_content(item, "externallink", "")
                image2 = get_tag_content(item, "thumbnail", "")
                fanart2 = get_tag_content(item, "fanart", "<")
                if external_link and not external_link in ["http://no/", "http://nolink", "http://nolink/"]:
                    url2 = external_link                
                if fanart2 == '<':
                    fanart2 = FANART
            ...
            HTML3 = getUrl('http://ask4access.com/ask-kodi/_v4/search.xml')
            items = re.compile("<item>(.+?)</item>", re.DOTALL).findall(HTML3)
            for item in items:
                name3 = get_tag_content(item, "title", "")
                url3 = get_tag_content(item, "link", "")
                external_link = get_tag_content(item, "externallink", "")
                image3 = get_tag_content(item, "thumbnail", "")
                fanart3 = get_tag_content(item, "fanart", "")
                info3 = get_tag_content(item, "info", "")
                if external_link and not external_link in ["http://no/", "http://nolink", "http://nolink/"]:
                    url3 = external_link
                if fanart3 == '<':
                    fanart3 = FANART
...
def get_tag_content(collection, tag, default):
    try:
        result = re.findall('<%s>(.+?)</%s>' % (tag, tag), collection, re.DOTALL)[0]
        return result
    except:
        return default
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR],New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('[COLOR lime]Select A Source[/COLOR]', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
def getItems(items,fanart):
        ...
        for item in items:
            ...
            try:
                url = []
                if name == 'SearchAddon':
                    addDir('[COLOR white][B]Search Addon[/B][/COLOR]','Search',41,'http://i.imgur.com/shDL3f4.png' ,  FANART,'','','','')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR],Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification([COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR],Loading Content.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR]','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR]', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from [COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to [COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        headers = dict({'User-Agent':'ask','Referer': 'http://joker.org/','Accept-Encoding':'gzip, deflate','Content-Type': 'application/json;charset=utf-8','Accept': 'application/json, text/plain, */*'})
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification([COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR],No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from [COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to [COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
def TextBoxesPlain(announce):
	class TextBox():
		...
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('[COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR]') # set heading
...
def AUTHENTICATE():
    ...
    if not os.path.exists(DATA_SETTINGS_FILE):
        xbmcgui.Dialog().ok("[COLOR blue][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] Reoladed[/COLOR]", "You are required to login to use [COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] ", "Please enter your login details at the next prompt.","Visit http://www.ask4access.com/members to sign up!")
    ...
    url = base64.b64decode(b'aHR0cDovL3d3dy5hc2s0YWNjZXNzLmNvbS9tZW1iZXJzL2NoZWNrLnBocD9lbWFpbD0=') + username + '&pass=' + password
    ...
    try:
        ...
        xbmcgui.Dialog().ok("[COLOR blue][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] ","[COLOR red][B]IMPORTANT INFORMATION:[/B][/COLOR]", "[COLOR yellow]There was an error connecting to the authentication server. Please try again later![/COLOR]")
        sys.exit(0)
    ...
    if str(total) != '1':
        xbmcgui.Dialog().ok('[COLOR blue][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR] Authentication Failure', 'We were unable to authenticate the log in details provided.','Please signup here: http://www.ask4access.com/members')
        sys.exit()

def Borrowers(url,fanart):
    print 'borrowers',url
    SetViewLayout = "List"
    soup = getSoup(url)
    #print type(soup)
    items = re.findall(".*url=\"(.*?)\">name=\"(.*?)\"", soup.text)
    for linkedUrl, name in items:
        addDir(name.encode('utf-8'),linkedUrl.encode('utf-8'),667,"","","","","",None,'source')

def Borrowers2(url,fanart):
    print 'borrowers',url
    SetViewLayout = "List"
    soup = getSoup(url)
    #print type(soup)
    ...
    for item in items:
        ...
        if href == "../":
            continue
        elif href.endswith(".mkv"):
            Play(name.encode('utf-8'),href.encode('utf-8'),58,"","","","","",None,'source')
        else:
            addDir(name.encode('utf-8'),href.encode('utf-8'),667,"","","","","",None,'source')
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR],Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification([COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]A[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR red][B]S[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR][COLOR cyan][B]K[/B][/COLOR][COLOR yellow][B]•[/B][/COLOR],Please [COLOR blue]install Youtube-dl[/COLOR] module ,10000,"")")
...
elif mode==63:Main_Menu()
elif mode==56:Index_List()
elif mode==57:Source_File2()
elif mode==58:resolve(url)
elif mode==59:Main_Loop(url)
elif mode==60:Write_Favourite(name,url,choice,fav_mode)
elif mode==61:Read_Favourite(name,url,choice,fav_mode)
elif mode==62:Favourites_menu()
elif mode==666:Borrowers(url,fanart)
elif mode==667:Borrowers2(url,fanart)
elif mode==668:Open_settings()

...
def forceUpdate():
	...
	LogNotify(ADDONTITLE, 'Forcing All Addon Updates')
...
def backUpOptions(type, name=""):
	...
	if type == "build":
		if DIALOG.yesno(ADDONTITLE, "Are you sure you wish to create a backup?", nolabel="No, Cancel", yeslabel="Yes, Continue"):
			if name == "":
			...
			DP.create("%s: Creating Backup" % ADDONTITLE, "Creating back up zip", "", "Please Wait...")
			for base, dirs, files in os.walk(HOME):
				dirs[:] = [d for d in dirs if d not in exclude_dirs]
				files[:] = [f for f in files if f not in exclude_files]
				for file in files:
			...
			DP.close()
			DIALOG.ok(ADDONTITLE, "[COLOR lawngreen]%s[/COLOR] backup successful:" % name, "[COLOR yellow]%s[/COLOR]" % zipname)
	elif type == "guifix":
		if name == "":
			guiname = Get_Keyboard("","Please enter a name for the %s backup" % type)
			...
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if yes:
						...
						LogNotify(ADDONTITLE,'Clear Packages: [COLOR lawngreen]Success[/COLOR]!')
...
def killxbmc():
	choice = DIALOG.yesno('Force Close/Restart', 'You are about to close/restart \n you need to do this after updating to a new version', 'Would you like to continue?',yeslabel='Yes, Close')
	if choice == 0: return
	elif choice == 1: pass
	myplatform = platform()
	log("Platform: " + str(myplatform))
	os._exit(1)
	log("Force close/restart has failed!  Trying alternate methods.")
	if myplatform == 'osx': # OSX
		...
		DIALOG.ok("[COLOR=red][B]WARNING !!![/COLOR][/B]", "If you\'re seeing this message it means the force close", "was unsuccessful. Please unplug the power and restart your box[COLOR=lime]DO NOT[/COLOR] exit cleanly via the menu.",'')
		...
		DIALOG.ok(ADDONTITLE, "[COLOR=lawngreen][B]Press the HOME button on your remote or unplug the power and re-launch...[/COLOR][/B]")
